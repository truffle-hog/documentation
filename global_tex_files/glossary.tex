...
\newglossaryentry{executor}{
    name=Executor,
		plural=Executors,
    description={Der Service der die \gls{programname} Befehle ausführt und sommit das Model verändert. Mehr im \hyperref[subsubsec:executor]{executor}-Package}.
}

\newglossaryentry{replaylog}{
    name=ReplayLog,
		plural=ReplayLogs,
    description={Ein Objekt das ein Snapshot von dem Graphen zu einem bestimmten Zeitpunkt mit einer Liste an \glspl{command} beinhaltet. Dieses Objekt wird
	  von Java serialisiert und dann auf der Festplatte gespeichert um zu einem spätteren Zeitpunkt in der Lage zu sein den Graphen zu rekonstruieren. Mehr im \hyperref[subsec:replaylogging]{replaylogging}-Package}.
}
